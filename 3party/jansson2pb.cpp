 ...
 *
 * jannson2pb is free software; you can redistribute it and/or modify
...
// FIXME: moved j2pb_error to header file
...
std::string pb2json(const Message &msg)
{
	std::string r;
        ...
        json_autoptr _auto(_pb2json(msg)); //FIXME <- used auto to fix memory leak
	json_dump_callback(_auto.ptr, json_dump_std_string, &r, 0); //FIXME <- used auto to fix memory leak
	return r;
}
